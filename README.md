...
A watch that ends for a transport reason (exit 1 with `reason` on the result line) first emits one
`watch-ended` line addressed to its own bearer — `reason`, `cursor`, and `re_arm`, the exact command
to arm it again — on stdout under `--json` and on stderr otherwise; under a Codex bridge the same
notice is queued as one turn, attempted once. A watch that ends normally emits none. The Codex
launchers' `--status` / `-Status` carry the last such line as `ended` once the armed pid is gone.
