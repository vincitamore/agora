...
- `interval`, `threadInterval`, `followCap`, `followIdleMinutes`, `pollBudget`, `note`. `followCap` is how many threads one session follows in one room at once (default 16). A busy room wants more; the cost is reads, `sessions × followed × 60/threadInterval` a minute, which `agora doctor` reports separately from room-history reads. The cap never takes a thread this session rooted, or one a human has just replied in, while any other thread is free.
