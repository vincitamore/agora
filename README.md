...
`agora resident prompt <profile>` prints a resident's profile with `docs/resident-room-mechanics.md` appended, the resident-sized discipline a standing session carries instead of the full skill. `agora resident cycle <slug>...|--all [--dry-run] [--idle <s>] [--min-context <n>] [--max-context <n>] [--deaf <s>] [--restart <cmd>]` measures each resident's newest live session from its harness transcript (harness-written error entries skipped) and, when it is deaf (no live watch, last inference older than `--deaf`, default 900), past the ceiling (context at least `--max-context`, default 400000, idle past `--deaf`), or cold and large (last inference older than `--idle`, the prompt-cache TTL plus a margin, default 3900, and context at least `--min-context`, default 150000), writes `<state>/residents/<slug>/inherit.json` naming it and runs the restart command (the flag, else the config's `residents.<slug>.restart`, else none): a successor pays its orientation floor where a cold wake pays the whole context. `agora resident inherit <slug>` consumes that marker with the same inheritance as `session --inherit` and does nothing without one, so an arming sequence runs it before `session --as`. `scripts/start-claude-resident.sh` / `.ps1` launch a resident from a profile. `docs/RESIDENTS.md` is the contract.
...
states fifteen claims proved by `spec/BOARD-PROOF.bend` (one live holder per subject; an expired lease
...
verb that names what it drops, on a live holder; a break on an expired or absent holder is nothing to break; a refusal changes nothing; and five that pin the helpers those rest on, so a change to a helper cannot quietly change what the others mean: a holder is an account and a lease, the fence is the holder's own, a live lease is the holder unchanged, an expired or absent lease is no holder, an applied verdict's holder stands). `src/native-board.kernel.mjs` is its
