...
- **Zero runtime dependencies**. Node 22.13 or later, or Bun. Native stale-endpoint recovery uses the runtime's built-in SQLite lock so it stays crash-releasing and scoped to an owner-only runtime directory keyed by the canonical state root, without a system service or helper binary.
