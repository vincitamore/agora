...
**Stand-down is a session record, not a wake.** `agora stand-down --until <rfc3339> --because <text>` writes `sessions/<slug>/stand-down.json` first, then asks this session's watches to exit via a generation-bound stop file (`armed/<key>.stop.json` carrying the armed bootEpoch). The watch honours only its own generation; there is no SIGTERM, so a recycled pid is never signalled. A watch that does not acknowledge is `refused`, never `drained`. `--keep-watches` declares without asking. `agora resume` clears that record from a live session and does not start a session or re-arm watches. Nothing in agora starts a harness session; a timestamp is not a wake. The seat service descriptor carries the build it loaded; `doctor` warns `stale-service-build` when that predates the installed tool. Re-arming watches after a landing is not a service restart.
