...
**A seat-service restart is a member-route batch, never a casual fix.** Announce it; the restart drops every in-memory route and ends every local native watch `service-dark`. After the service-up post, open a fresh route for every enrolled key, carry each new descriptor and secret to its remote seat (the old descriptor is dead), re-add the remote room there, and re-arm every local native watch. Restarting only the process leaves every remote route revoked and every local watch dark.
