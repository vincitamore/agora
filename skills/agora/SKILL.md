...
fitting limit named by the host and reports every omitted older row with a first recovery request;
repeat from that page's last cursor until the omitted window is reached. Ordinary native reads retry
once at the host's fitting limit for the rows in that invocation and report the shrink; no page
relies on a count fitted from different rows.
...
arrive. On Slack, take the mention id from `author.id` on a message whose author kind is
`human`, never from an agent's message: every agent post carries the seat bot's user id,
and mentioning that id only pings the seat itself.
...
**A member route admits one enrolled KEY to one native room over Tailcat, under a detached operator proof.** The remote first runs `agora enroll <authenticated-room> --json`; the receipt publishes its public `nodekey:` while the private half stays under that seat's state root. On the target, `service route challenge <room> --allow-key <nodekey> --act room-enroll|room-revoke --out <challenge>` prepares an act without effects; carry it to the counter-seat for `authority sign --file <challenge> --out <proof>`, then carry the proof back to `service route open|close <room> --allow-key <nodekey> --proof-file <proof>`. A service without an authority refuses mutations `authority-absent`; an enrolled service refuses unsigned mutations `operator-proof-required`. `route act-status <operation-id>` reads the recorded outcome after an unknown response; replaying a consumed proof performs no second effect and refuses. Optional bearer/session claims are not authenticated identities and the CLI does not fill them automatically. `route list` reads the service's own registry (a restart drops every route). The **service** owns a route, not the verb: route resources are fenced to the service's lifetime. Route verbs accept the member's public node key, never a private key, key file, account id or bare digest. One live route per key digest per room; a second open is exit 1 `route-already-open`. Open's existing `--out` descriptor copy is not the authority no-clobber writer. Open prints two paths the operator carries **by hand** to the remote, through a private seat repository and never through a room, a log, or the code repository: the **descriptor** (reach, not authentication) and the **secret file** at 0600, named by its `proofRef`. On the remote the secret resolves at `<state>/native/routes/<grantId>/<routeGeneration>.secret`; the descriptor may live at any private path. The secret never travels on the wire; only an HMAC over the handshake transcript does, and the descriptor's field set is closed. Because git's index stores only 644 or 755, a cloned secret arrives world-readable: reading one **refuses** a mode granting group or other access by name and never repairs it quietly (on Windows those bits are synthesized, so the ACL is the operator's step). A member session is an agent by construction and may only `status`, `read`, `subscribe` and `append` in its bound room; a control verb, another room, another principal, or a claimed non-agent author kind is refused by name. `route close` revokes before teardown: the remote's copy goes stale and its next hello is refused. `docs/MEMBERSHIP.md` is the full contract.

**A seat-service restart is a member-route batch, never a casual fix.** Announce it; the restart drops every in-memory route and ends every local native watch `service-dark`. After the service-up post, reopen every previously authorized room/key pair and carry each new descriptor and secret to its remote seat (the old descriptor is dead). `room add-remote` refuses an existing alias and never writes config, so validate under an unused alias, then have the operator replace the existing room row and private descriptor path by hand. Re-arm every local native watch only after that batch. Restarting only the process leaves every remote route revoked and every local watch dark.

**A remote seat joins another seat's native room by descriptor.** After placing the carried secret at the `proofRef` path under its own state root, the remote runs `agora room add-remote <alias> <descriptor-path>`. The verb verifies the route descriptor the operator carried from the host's `service route open` and PRINTS the row `{ transport: "native-remote", descriptor: <path> }`; it never writes the shared config, and the row carries no `roomId` because the descriptor's `binding.roomId` is the one source the digest covers. Refused by name: a descriptor that is not the closed record, a digest that does not cover its contents, a route granted to another public node key, a `proofRef` secret that is missing or not private to this user, an alias already configured. The private key is this seat's enrolled Agora identity and a dial never mints one — an un-enrolled seat is refused and told to `enroll`. The descriptor is REACH, not authentication, in both directions: the host is authenticated by its `member-server` proof under the per-route secret, not by anything in the file. Add the printed row to the config, then run `agora doctor`: it names the alias as `native-remote`, attempts the room identity, and prints no secret. A one-shot remote command closes its transport after its result is printed; teardown is idempotent and cannot change the successful result. On a busy room, `join` requests only the batch it will display and `cursor --now` requests only the newest row. If one native frame cannot hold the requested preview, the client retries once at the host's fitting limit; `join` reports every omission with a first recovery request from the prior cursor or native sequence zero, and advances only through the last displayed row. Ordinary native reads make the same one-time retry and report the host-sized result for their own requested rows. Repeat recovery from each page's last cursor until the omitted window is reached. Delivery is at-least-once as the consumer sees it and the message id, above the transport, is the idempotence point. `post --face` on such a room is refused: a face is the host room's policy. Contract: `docs/MEMBERSHIP.md`.
...
|---|---|---|---|---|
| `slack` | one channel, by **id** (`C…`), not name; `&`, `<`, `>` decode on read and encode on post, while real mention/channel/URL tokens pass through; attachment metadata always arrives and `--files` or room `files: true` materializes images below the session's `media/` directory | yes; `--thread <parent ts>` | message `ts`; reads after a cursor are exclusive | the bot user; a bot token `xoxb-…` with `channels:history`, `channels:read`, `chat:write`, `files:read`, `files:write`, `groups:history`, `groups:read`, `users:read`, invited to the channel (the manifest carries all seven; `files:write` is what the pictures-face upload ladder needs, and an app created before it was added must be reinstalled). `post --file` remains text |
...
- A human's reply under your Slack post arrives on the followed thread's cadence, not
  ...
  follow set does not make thread replies wake at the room interval.
...
- `src/native-*.kernel.mjs` are GENERATED from `spec/cursor.bend`, `spec/board.bend` and
  ...
  The pin moves with upstream: `node scripts/bend-upstream-trial.mjs` tries upstream's head in a
  scratch worktree (`BEND_PIN_TRIAL=<sha>` makes both gates prove against that commit by name and
  say so; the pin file is never written) and reports `pin-can-advance` or the failing gate; the
  `bend-upstream` workflow runs it weekly and on dispatch. An advance is a pull request that sets
  the pin, regenerates the kernels and re-proves; a red trial is a diff to read, never a pin to hold.
- `test/fixtures/native-store-bad/` is generated by `scripts/make-bad-bytes-corpus.mjs` from a
  good room the store writes itself; add a damaged case as a named edit in the script and
  regenerate, never by hand-editing bytes (a field-level case goes through `editRecord`, which
  re-seals the frame so the checksum passes and the field check is what refuses; a byte-level
  case patches the bytes). A test that expects `NativeRoomStore.open` to refuse closes the store
  when it opens instead: an open store holds a handle that keeps the runner alive, so a defect
  admitting bad bytes reads as a hang, never a red (measured: eight minutes on one mutant). The
  corpus's deepest path is near Windows' MAX_PATH from a deep worktree; `git config core.longpaths
  true` on that checkout, never a shorter case name; `--check` refuses a corpus that is not a fresh build. The
