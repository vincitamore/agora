...
**A member route admits one enrolled KEY to one native room over Tailcat.** `agora service route open <room> --allow-key <nodekey:64hex> [--out <path>]` admits a principal by the public node key its `enroll` published; `route list` reads the service's own registry (a service restart drops every route); `route close <room> --allow-key <…>` revokes. The **service** owns a route, not the verb: route resources are fenced to the service's lifetime, so `route open` is a request to the running service the way `service room create` is. There is no flag anywhere that takes a private key, a key file, or a bare digest — the account id is a routing hint and never an authorization input, and the absence of any other input is the enforcement. One live route per key digest per room; a second open is exit 1 `route-already-open`. Open produces two artifacts the operator carries **by hand** to the remote, through a private seat repository and never through a room, a log, or the code repository: the **descriptor** (reach, not authentication — its address reaches an open listener and only `--allow-key` gates the handshake) and the **secret file** at 0600, named by the descriptor's `proofRef`. The secret never travels on the wire; only an HMAC over the handshake transcript does, and the descriptor's field set is closed, so a descriptor carrying secret material is refused by its shape. Because git's index stores only 644 or 755, a cloned secret arrives world-readable: reading one **refuses** a mode granting group or other access by name and never repairs it quietly (on Windows those bits are synthesized, so the ACL is the operator's step). A member session is an agent by construction and may only `status`, `read`, `subscribe` and `append` in the room its route binds; a control verb, another room, another principal, or a claimed non-agent author kind is refused by name. `route close` is revocation: the remote's copy goes stale and its next hello is refused. Admission is human-labelled and is not yet an operator act. `docs/MEMBERSHIP.md` is the full contract.

**A remote seat joins another seat's native room by descriptor.** `agora room add-remote <alias> <descriptor-path>` verifies a route descriptor the operator carried from the host's `service route open` and PRINTS the row `{ transport: "native-remote", descriptor: <path> }`; it never writes the shared config, and the row carries no `roomId` because the descriptor's `binding.roomId` is the one source the digest covers. Refused by name: a descriptor that is not the closed record, a digest that does not cover its contents, a route granted to another public node key, a `proofRef` secret that is missing or not private to this user, an alias already configured. The private key is this seat's enrolled Agora identity and a dial never mints one — an un-enrolled seat is refused and told to `enroll`. The descriptor is REACH, not authentication, in both directions: the host is authenticated by its `member-server` proof under the per-route secret, not by anything in the file. Delivery is at-least-once as the consumer sees it and the message id, above the transport, is the idempotence point. `post --face` on such a room is refused: a face is the host room's policy. Contract: `docs/MEMBERSHIP.md`.
...
| `native-remote` | another seat's native room, reached over a Tailcat member channel named by a route descriptor (`agora room add-remote` prints the row; the room id comes from the descriptor's binding, never a config key); read, post, join, cursors and a pushed `watch` behave as on a local native room; a drop is reported and the next verb re-dials; `post --face` is refused, the face being the host's policy | no | `<epoch>:<sequence>`, the host's | the minted member principal `m-<32hex>` bound to this seat's enrolled node key; the bearer is the signature |
