...
const AUTH = /** @type {const} */ (["auth.test", () => ({ body: { ok: true, user_id: "UBOT", user: "claude-bot" } })]);
/** @param {string} s */
...
test("fixture 03 arm 2: the reconciliation read finds the rider, so the face is published with that ts and zero posts", async () => {
  ...
  const r = await rig({ clock: c, records: [unknownA], routes: [AUTH, ["conversations.history", () => ({ body: { ok: true, has_more: false, messages: [
    { ts: "1788609601.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "the derper is up on 443 and 3478", metadata: { event_type: "agora_face", event_payload: { origin: ORIGIN_A } } },
  ] } })]] });
  try {
...
test("fixture 03 arm 5: no rider and two byte-identical bodies from this seat stay unknown, quarantined, listed for a human", async () => {
  const c = clock("2026-09-05T12:01:00.000Z");
  const r = await rig({ clock: c, records: [unknownA], routes: [AUTH, ["conversations.history", () => ({ body: { ok: true, has_more: false, messages: [
    { ts: "1788609601.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "ack" },
    { ts: "1788609602.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "ack" },
...
test("fixture 03 arm 5 (distinct bodies): no rider but a unique payload reconciles exactly through the seat's own account", async () => {
  ...
  const r = await rig({ clock: c, records: [unknownA], routes: [AUTH, ["conversations.history", () => ({ body: { ok: true, has_more: false, messages: [
    { ts: "1788609601.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "ack" },
    { ts: "1788609602.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "nack" },
...
test("fixture 05: a human's Slack thread reply lands in the native thread of the published face, origin stamped by the service", async () => {
  ...
  const parent = { ts: "1756900001.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "the derper is up on 443 and 3478", thread_ts: "1756900001.000000", reply_count: 1, metadata: { event_type: "agora_face", event_payload: { origin: ORIGIN_A } } };
...
const fixture07History = [
  { ts: "1756900001.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "the derper is up on 443 and 3478", metadata: { event_type: "agora_face", event_payload: { origin: ORIGIN_A } } },
  { ts: "1756900014.900000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "ack", metadata: { event_type: "agora_face", event_payload: { origin: ORIGIN_B } } },
...
test("fixture 07 (not author based): a message from our own bot account with no face record and no pending face IS ingested", async () => {
  const r = await rig({ records: [publishedA], routes: [AUTH, ["conversations.history", () => ({ body: { ok: true, has_more: false, messages: [
    { ts: "1756900030.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "posted by the other seat through the same token" }] } })]] });
  try {
    /** @type {any[]} */
    const appended = [];
    const polled = await r.runner.poll(r.face, { read: reader(r), appendForeign: async (m) => { appended.push(m); } });
...
test("fixture 07 rule 3: our own account while a face is pending inside the settle window is HELD, not ingested and not dropped, and the cursor is withheld", async () => {
  ...
  const r = await rig({ clock: c, records: [pendingB], routes: [AUTH, ["users.info", () => ({ body: { ok: true, user: { id: "U2", real_name: "Alex" } } })],
    ["conversations.history", () => ({ body: { ok: true, has_more: false, messages: [
      { ts: "1756900014.900000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "ack" },
...
test("fixture 10: under pictures, the image is uploaded once through files:write with W10's bounds, the pcap is metadata only, no path and no bytes reach the text", async () => {
  ...
  file = r.file;
  try {
    const message = msg({ text: "here is the collector\n\nto: Alex\n\n-- Grace", attachments: [{ ...image, path: "C:\\Users\\alex\\blobs\\7f3a.png" }, pcap] });
    const { faces, settled } = await r.runner.face(message);
    assert.deepEqual(faces, [{ transport: "slack", status: "pending" }]);
...
test("fixture 10 / 03: a lost completeUpload response is unknown, reconciled by file id in the channel, never re-shared blind", async () => {
  ...
  const r = await rig({ clock: c, face: { attachments: "pictures" }, readBlob: async () => PNG,
    ...
    routes: [...uploadRoutes({ complete: () => { if (lose) { lose = false; throw new TypeError("fetch failed"); } return { body: { ok: true, files: [{ id: "F0PIC" }] } }; } }),
      ["conversations.history", () => ({ body: { ok: true, has_more: false, messages: [
        { ts: "1756900001.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "pic", metadata: { event_type: "agora_face", event_payload: { origin: ORIGIN_A } } },
        { ts: "1756900001.500000", bot_id: "B1", subtype: "file_share", username: "claude-bot", text: "", files: [{ id: "F0PIC", name: "screen shot.png" }] }] } })]] });
  try {
    const { settled } = await r.runner.face(msg({ text: "pic\n\nto: Alex\n\n-- Grace", attachments: [image] }));
    const after = await settled;
...
test("fixture 12 (ambiguous): the candidates are quarantined and never ingested, the rest of the room ingests normally", async () => {
  ...
  const r = await rig({ clock: c, records: [{ ...staleA, payloadDigest: sha("ack") }], routes: [AUTH, ["users.info", () => ({ body: { ok: true, user: { id: "U2", real_name: "Alex" } } })],
    ["conversations.history", () => ({ body: { ok: true, has_more: false, messages: [
      ...
      { ts: "1756900002.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "ack" },
      { ts: "1756900001.000000", bot_id: "B1", subtype: "bot_message", username: "claude-bot", text: "ack" }] } })]] });
  try {
    /** @type {any[]} */
    const appended = [];
    const polled = await r.runner.poll(r.face, { read: reader(r), appendForeign: async (m) => { appended.push(m); } });
...
const comment = (id, body, o = {}) => ({ id, body, created_at: o.at ?? "2026-09-05T12:00:01Z", updated_at: o.at ?? "2026-09-05T12:00:01Z", user: { login: o.login ?? "vincitamore", id: o.uid ?? 7, type: o.type ?? "User" }, html_url: `https://github.com/example-org/example-repo/issues/3#issuecomment-${id}` });
...
test("github / fixture 07 and 05: a comment from a human is ingested with a validated OriginReference and an unverified key, an own echo by id is not, our own login with no record IS, and our own login while a face is pending is held", async () => {
  ...
  try {
    ...
    assert.deepEqual(appended.map((m) => [m.origin.source.id, m.author.id, m.author.kind, m.key, m.signedAs]), [["4243", "peer", "human", "unverified", undefined], ["4244", "vincitamore", "human", "unverified", undefined], ["4245", "app[bot]", "agent", "unverified", "Codex/ops"], ["4246", "peer", "human", "unverified", undefined]], "a body that quotes an origin id is a body, never a rider: the comment is foreign and ingested");
