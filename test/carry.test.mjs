...
test("carry: only this session's own posts are folded, and the ledger is the whole filter", () => {
  ...
  const mine = foldRoom(all, new Set(["m1", "m2"]), { bearer: "Alice/watch" });
...
test("carry: a released claim leaves the open list and the release is carried beside it", () => {
  const c = foldRoom([OWN_CLAIM, OWN_SECOND, OWN_RELEASE], new Set(["m1", "m2", "m3"]), { bearer: "Alice/watch" });
...
test("carry: the earliest claim on a subject is the one carried", () => {
  ...
  const c = foldRoom([OWN_CLAIM, again], new Set(["m1", "m9"]), { bearer: "Alice/watch" });
...
test("carry: a verdict carries its exhibits, and an obligation is an own post with a to:", () => {
  const c = foldRoom([OWN_CLAIM, OWN_VERDICT], new Set(["m1", "m4"]), { bearer: "Alice/watch" });
...
test("carry: at top level a delivery stays owed under a later own post, and an address for someone else is never owed", () => {
  ...
  const c = foldRoom([before, OWN_CLAIM, THEIRS, elsewhere], new Set(["m1"]), { bearer: "Alice/watch" });
  ...
  assert.deepEqual(c.owed.map((o) => [o.from, o.to]), [["Peer/dev", ["Alice/watch"]], ["Peer/dev", ["Alice"]]]);
...
test("carry: a receipt naming A leaves B owed, and a later unrelated top-level post leaves both owed", () => {
  ...
  const c = foldRoom([a, b, receipt], new Set(["m22"]), { bearer: "Alice/watch" });
  ...
  const both = foldRoom([a, b, later], new Set(["m23"]), { bearer: "Alice/watch" });
...
test("carry: with nothing of this session's own in the window, every addressed message is owed", () => {
  const c = foldRoom([THEIRS], new Set(), { bearer: "Alice/watch" });
...
test("carry: an address naming the seat, the model or everyone reaches this bearer", () => {
  ...
  const c = foldRoom([seat, named], new Set(), { bearer: "Alice/watch", seat: { id: "U01", name: "agora-bot" } });
...
test("carry: the window folds the room's live threads, so a release posted as a thread reply closes its claim", async () => {
  ...
  const c = foldRoom(w.messages, new Set(["p1", "r3"]), { bearer: "Alice/watch" });
  ...
  const blind = foldRoom(plain.messages, new Set(["p1", "r3"]), { bearer: "Alice/watch" });
...
test("carry: a thread the transport cannot read is named in threadsUnread and the rest still fold", async () => {
  ...
  const p1 = msg("p1", "one\n\nclaim: a\n\n-- Alice/watch", { cursor: "10", raw: { reply_count: 1, latest_reply: "11" } });
  const p2 = msg("p2", "two\n\nclaim: b\n\n-- Alice/watch", { cursor: "20", raw: { reply_count: 1, latest_reply: "31" } });
  const p3 = msg("p3", "three\n\nclaim: c\n\n-- Alice/watch", { cursor: "30", raw: { reply_count: 1, latest_reply: "41" } });
  const r1 = msg("r1", "handing a back\n\nrelease: a\n\n-- Alice/watch", { thread: "p1", cursor: "11" });
  const r3 = msg("r3", "handing c back\n\nrelease: c\n\n-- Alice/watch", { thread: "p3", cursor: "41" });
  /** @type {string[]} */
  const order = [];
  const transport = {
    threads: true,
  ...
  const c = foldRoom(w.messages, new Set(["p1", "p2", "p3", "r1", "r3"]), { bearer: "Alice/watch" });
...
test("carry: a malformed thread id and a thread that is gone are recorded, never thrown", async () => {
  ...
  const mangled = msg("m5", "a reply whose thread lost its last digits\n\n-- Alice/watch", { thread: "178845964", cursor: "9" });
  const gone = msg("p9", "two\n\n-- Alice/watch", { cursor: "20", raw: { reply_count: 1, latest_reply: "21" } });
  const transport = {
    threads: true,
    /** @param {string} id */
...
test("carry: through the slack transport, a thread whose ts lost its digits is named unread with the reason and never read", async () => {
  ...
  const text = renderCarry({ ...foldRoom(w.messages, new Set(), { bearer: "Alice/watch" }), threadsUnread: w.threadsUnread, cursorKey: "down", cursor: null, threads: w.threads });
...
test("carry: a claim taken again after a release re-opens the subject, and the earliest after the release holds", () => {
  ...
  const c = foldRoom([take, give, again, later], new Set(["c1", "c2", "c3", "c4"]), { bearer: "Alice/watch" });
...
test("carry: a verdict answering an earlier own verdict supersedes it, and the withdrawn one is carried", () => {
  ...
  const c = foldRoom([first, withdrawn], new Set(["v1", "v2"]), { bearer: "Alice/watch" });
  ...
  const b = foldRoom([first, unrelated], new Set(["v1", "v3"]), { bearer: "Alice/watch" });
...
test("carry: a withdraws: supersedes an earlier own verdict and releases an earlier own claim", () => {
  ...
  const c = foldRoom([first, back], new Set(["v1", "v2"]), { bearer: "Alice/watch" });
  ...
  const claims = foldRoom([OWN_CLAIM, OWN_SECOND, dropped], new Set(["m1", "m2", "m20"]), { bearer: "Alice/watch" });
  ...
  const foreign = foldRoom([first, OWN_CLAIM, msg("v4", "x\n\nwithdraws: foreign-9\n\n-- Alice/watch")], new Set(["v1", "m1", "v4"]), { bearer: "Alice/watch" });
...
test("carry: an incoming withdraws: is rendered and never folded", () => {
  ...
  const c = foldRoom([mine, theirs], new Set(["v1"]), { bearer: "Alice/watch" });
...
test("carry: a delivery addressed here is owed until this session answers it by name", () => {
  ...
  const open = foldRoom([OWN_CLAIM, ask], new Set(["m1"]), { bearer: "Alice/watch" });
  ...
  const receipt = msg("m11", "on it\n\nre: m10\n\n-- Alice/watch", { thread: "t9" });
  const answered = foldRoom([OWN_CLAIM, ask, receipt], new Set(["m1", "m11"]), { bearer: "Alice/watch" });
  assert.deepEqual(answered.owed, []);

  const other = msg("m11", "on something else\n\nre: m99\n\n-- Alice/watch", { thread: "t9" });
  const still = foldRoom([OWN_CLAIM, ask, other], new Set(["m1", "m11"]), { bearer: "Alice/watch" });
...
test("carry: nothing rendered carries a message body", () => {
  const c = foldRoom([OWN_CLAIM, OWN_VERDICT, THEIRS], new Set(["m1", "m4"]), { bearer: "Alice/watch" });
  const text = renderCarry({
    room: { alias: "down", transport: "local", room: "/tmp/down.ndjson" },
    bearer: { name: "Alice/watch", source: "session" },
  ...
  assert.match(text, /owed {8}from Peer\/dev to Alice {3}m5 cursor 5/);
...
test("cli: carry derives the keep-list from the session directory and one bounded read", async () => {
  ...
  try {
    ...
    assert.deepEqual(c.bearer, { name: "Alice/watch", source: "session" });
...
async function slackStub(o = {}) {
  const parents = [
    { ts: "1788459640.000100", user: "U1", text: "taking a\n\nclaim: a\n\n-- Alice/watch", reply_count: 1, latest_reply: "1788459640.000110" },
    { ts: "1788459640.000200", user: "U1", text: "taking b\n\nclaim: b\n\n-- Alice/watch", reply_count: 1, latest_reply: "1788459640.000210" },
    { ts: "1788459640.000300", user: "U1", text: "taking c\n\nclaim: c\n\n-- Alice/watch", reply_count: 1, latest_reply: "1788459640.000310" },
  ];
  ...
  const server = createServer((req, res) => {
    ...
    if (url.pathname === "/users.info") return send({ ok: true, user: { real_name: "Alice" } });
...
test("cli: session --inherit --dry-run says what it would copy and copies nothing", async () => {
  ...
  try {
    ...
    await agora(["session", "--as", "Alice/watch"], one);
