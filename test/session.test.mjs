...
test("bearer: --as, then AGORA_ACTOR, then the config; paths validated at the boundary", () => {
  assert.deepEqual(resolveBearer(cfg, { as: "Alice/watch", env: { AGORA_ACTOR: "Opus" } }), { name: "Alice/watch", source: "--as" });
  ...
  assert.throws(() => resolveBearer(cfg, { env: { AGORA_ACTOR: "Alice/" } }), /bearer path/);
  assert.throws(() => resolveBearer(cfg, { env: { AGORA_ACTOR: "Alice watch" } }), /bearer path/);
...
test("every state write is a rename into place: nothing half-written, no temp file left behind", async () => {
  ...
  try {
    ...
    await writeRecord(sdir, s, { bearer: "Alice/watch", pid: process.pid, pidSource: "TEST" });
    ...
    assert.equal((await readRecord(sdir))?.bearer, "Alice/watch", "the record parses whole");
...
test("the session record: written once, bearer replaced on re-register, startedAt kept, touched on later calls", async () => {
  ...
  try {
    ...
    const first = await writeRecord(sdir, s, { bearer: "Alice/watch", label: "the watch", pid: 4242, pidSource: "CLAUDE_PID", build: oldBuild, now: new Date("2020-01-01T00:00:00Z") });
...
test("liveness: a matching boot and an answering pid is live; ESRCH or a reboot is gone; EPERM counts as alive; no pid is unknown", () => {
  const rec = { slug: "s", source: "x", bearer: "Alice", pid: 100, bootEpoch: 1000, startedAt: "t", lastSeen: "t" };
...
test("listRecords: every session with state, registered or not, with its liveness", async () => {
  ...
  try {
    ...
    await writeRecord(sessionDir(dir, a), a, { bearer: "Alice/watch", pid: process.pid, pidSource: "TEST" });
...
test("departures: gone past the grace and within the stale horizon, not yet announced in this room, never oneself; one announcer wins", async () => {
  ...
  try {
    ...
    const dead = () => { const e = /** @type {NodeJS.ErrnoException} */ (new Error("gone")); e.code = "ESRCH"; throw e; };
    await mk("me", "Alice/watch", 1, 1);
    await mk("quiet", "Opus/design", 30, 2); // gone, quiet for 30 minutes: announce
    await mk("blip", "Alice/review", 1, 3); // gone but touched a minute ago: a restart, not a departure
    ...
    await mk("elsewhere", "Alice/build", 30, 5); // gone, but never touched this room: not this room's news
    ...
    assert.match(departureLine(gone[0].record, ["Alice/watch", "Codex"]), /^Opus\/design is no longer running \(last seen .*Z\)\. Requests addressed to it will not be answered; re-address them\. Still here on this seat: Alice\/watch, Codex\.$/);
    assert.match(departureLine(gone[0].record, []), /No other session is provably live/);
    assert.match(departureLine(gone[0].record, ["Alice/watch"], ["Grok-4.6/general"]), /Still here on this seat: Alice\/watch\. Also registered here, liveness not provable from this process: Grok-4\.6\/general\./, "an unprobeable bearer is named, never dropped");
...
test("one sweep is one post: a roster naming every bearer that went dark, and no count", async () => {
  ...
  const one = departuresLine([rec("Opus/design", "2026-09-04T11:20:00.000Z")], ["Alice/watch"]);
  ...
  const many = departuresLine(
    [rec("Alice/build", "2026-09-03T13:02:00.000Z"), rec("Alice/review", "2026-09-04T03:51:00.000Z"), rec("Opus/design", "2026-09-04T11:20:00.000Z")],
    ["Alice/orchestrator"],
    [{ bearer: "Grok-4.6/general", lastSeen: "2026-09-04T11:00:00.000Z" }],
  );
  assert.match(many, /^Alice\/build, Alice\/review and Opus\/design are no longer running \(last seen 2026-09-03T13:02:00Z, 2026-09-04T03:51:00Z, 2026-09-04T11:20:00Z, in that order\)\./);
  assert.match(many, /Requests addressed to them will not be answered; re-address them\. Still here on this seat: Alice\/orchestrator\./);
...
test("the identity line names bearer, session and their sources, and warns on a shared default with siblings", async () => {
  const { dir, cleanup } = await tmp();
  try {
    const bearer = { name: "Alice/watch", source: "AGORA_ACTOR" };
    let line = await identityLine(bearer, { slug: "default", source: "default", explicit: false }, dir);
    assert.equal(line, "agora: Alice/watch (from AGORA_ACTOR) · session default (from default)");
