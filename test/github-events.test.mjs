...
const events = [
  { id: "50", type: "PushEvent", actor: { login: "bone" }, repo: { name: "bonejohnson8/slopcannon" }, created_at: "2026-09-03T10:05:00Z",
  ...
  { id: "40", type: "PullRequestEvent", actor: { login: "vincitamore" }, repo: { name: "bonejohnson8/slopcannon" }, created_at: "2026-09-03T10:04:00Z",
    payload: { action: "opened", number: 14, pull_request: { number: 14, title: "Fix the escaped exception", html_url: "https://github.com/bonejohnson8/slopcannon/pull/14", head: { ref: "fix-escape" }, base: { ref: "main" } } } },
  { id: "30", type: "IssueCommentEvent", actor: { login: "codex[bot]" }, repo: { name: "bonejohnson8/slopcannon" }, created_at: "2026-09-03T10:03:00Z",
    payload: { action: "created", issue: { number: 3, html_url: "https://github.com/bonejohnson8/slopcannon/issues/3" }, comment: { body: "candidate is up\n\n-- Codex", html_url: "https://github.com/bonejohnson8/slopcannon/issues/3#issuecomment-1" } } },
  { id: "20", type: "CreateEvent", actor: { login: "bone" }, repo: { name: "bonejohnson8/slopcannon" }, created_at: "2026-09-03T10:02:00Z", payload: { ref_type: "branch", ref: "feature/x" } },
  { id: "10", type: "WatchEvent", actor: { login: "someone" }, repo: { name: "bonejohnson8/slopcannon" }, created_at: "2026-09-03T10:01:00Z", payload: { action: "started" } },
];
...
function make(room = {}) {
  ...
  const t = githubEventsTransport({ transport: "github-events", repo: "bonejohnson8/slopcannon", ...room }, { token: "ghp_x", fetch });
...
test("github-events room validates its scope", () => {
  ...
  assert.equal(githubEventsTransport({ transport: "github-events", org: "NCU" }, { token: "t" }).room, "org:NCU events");
});
...
test("github-events: events become messages, newest last, with actors, summaries, urls and signatures", async () => {
  ...
  assert.equal(msgs[4].text, "pushed 2 commits to main in bonejohnson8/slopcannon\n  bbbbbbb worker: retry the provider fetch\n  ccccccc tests");
  assert.equal(msgs[4].url, "https://github.com/bonejohnson8/slopcannon/compare/aaaaaaa1...bbbbbbb2");
  assert.equal(msgs[4].author.name, "bone");
  assert.equal(msgs[3].text, "opened pull request #14: Fix the escaped exception (fix-escape -> main) in bonejohnson8/slopcannon");
  assert.equal(msgs[3].url, "https://github.com/bonejohnson8/slopcannon/pull/14");
  ...
  assert.equal(msgs[1].text, "created branch feature/x in bonejohnson8/slopcannon");
  assert.equal(msgs[0].text, "WatchEvent started in bonejohnson8/slopcannon");
