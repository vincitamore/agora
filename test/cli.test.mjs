...
test("cli: two sessions in one state root each keep their own position and see each other's posts", async () => {
  ...
  try {
    ...
    const A = { AGORA_CONFIG: cfgPath, AGORA_STATE: root, AGORA_SESSION: "fable-a", AGORA_ACTOR: "Alice/watch" };
    const B = { AGORA_CONFIG: cfgPath, AGORA_STATE: root, AGORA_SESSION: "fable-b", AGORA_ACTOR: "Alice/review" };

    let r = await agora(["post", "down", "from a"], A);
    assert.equal(r.code, 0);
    assert.match(r.stderr, /^agora: Alice\/watch \(from AGORA_ACTOR\) · session fable-a \(from AGORA_SESSION\)/m, "the identity line is on stderr");
    ...
    assert.deepEqual([armLine.type, armLine.bearer, armLine.session], ["identity", "Alice/review", "fable-b"], "the arm is on stdout too, so a monitor that reads only stdout can check it");
