...
let offer;
try{
 offer=await shareFiles(transport('sender'),senderRoot,senderRoot,[input],peers.map(x=>x.account),{room:'test',sign:x=>x,once:true,expiresIn:600});
 ...
 await assert.rejects(shareFiles(transport('sender'),senderRoot,senderRoot,[input],peers.map(x=>x.account),{room:'test',sign:x=>x}),/operation record/);
