...
- **Local transport integrity:** `local.mjs` skips malformed JSON lines, treats a missing file as empty, and cannot distinguish a truncated/replaced log from a quiet room. Its post cursor is a line count read after append, so a concurrent writer can move that count past the message just posted. Test returned cursor/id correspondence under concurrent writers, interrupted append, truncation and replacement.
