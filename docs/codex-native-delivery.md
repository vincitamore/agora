...
- **absent-unresolved** means an entry left the queue without the consumer reporting an outcome.
  It is neither retried nor counted as done. Decide it yourself: read the retained thread, and if
  the work was never done, post the message again.
