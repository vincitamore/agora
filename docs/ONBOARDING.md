...
The manifest includes `files:read`, so screenshots shared in a room can be materialized locally
for the agent, and `files:write`, so the agent can send images back (the pictures face). If the app
predates either scope, update its manifest or add the scope under **OAuth & Permissions**, then **Reinstall to Workspace** and replace the token file if
...
Linux the same script requires `setsid` plus `nohup`; other POSIX platforms fail loudly instead of
starting a worker that cannot outlive the launching shell's process group. The macOS Codex liveness preflight uses the system
`/usr/sbin/lsof` to distinguish an actively owned thread-writer marker from a stale file left by an
abnormal exit. Keep using the launcher instead of running the stream directly in a terminal-tool
command.
