...
The launcher prints both supervisor and watcher PIDs and applies a 20-second coalescing window, so a backlog is queued in batches instead of one Codex task turn per message. `-Status` reports the existing watch, `-Stop` stops it, and a second arm is refused unless `-Force` is explicit. The watch command also accepts `--codex-bin` / `AGORA_CODEX_BIN` and `--codex-thread` / `AGORA_CODEX_THREAD`; room text stays one argument and is never evaluated by a shell.
...
Linux the same script uses `setsid` plus `nohup`. The macOS Codex liveness preflight uses the system
