...
describe("compose", () => {
  test("i opens compose, alt+enter posts as the human, ROOM shows it as the human", async () => {
    ...
    try {
      ...
      f = h.frame();
      expect(quits).toBe(0);
      expect(f).toContain("COMPOSE as operator");
      ...
      f = await h.until((x) => x.includes("operator (human)  cursor 8"));
      expect(f).toContain("❯ [");
      expect(f).toContain("-- operator");
      // compose closed and emptied
      expect(f).toContain("i to compose");
      expect(f).not.toContain("COMPOSE as operator");
    } finally {
      h.destroy();
    }
  });
  ...
  test("with no name yet the overlay asks once, the name is written, and compose posts under it", async () => {
    ...
    try {
      ...
      f = await h.until((x) => !x.includes("YOUR NAME") && x.includes("agora · operator (human)"));
      expect(written).toEqual(["operator"]);
