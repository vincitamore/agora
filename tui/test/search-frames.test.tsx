...
describe("SEARCH", () => {
  test("rows and a horizon line, never a count; enter jumps ROOM to the row", async () => {
    ...
    try {
      ...
      expect(f).toMatch(/Fable \(agent\)\s+c1\s+Starting the TUI slice/);
      expect(f).toMatch(/Fable \(agent\)\s+c3\s+Read\. The TUI imports/);
      ...
      expect(f).toContain("❯ [2026-09-05T01:05:00.000Z] Sol/codex (agent)  cursor 2");
